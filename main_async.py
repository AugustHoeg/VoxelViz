...
class OMEZarrOrthoViewer:
    def __init__(self, zarr_root_path):
        ...
        self.render_tasks = [None, None, None]  # one per axis

    def _compute_slice(self, axis, level0_index, level):
        ...
        # push conversion into dask graph
    ...
    async def render_slice(self, axis, level0_index, level):
        # cancel older task if still running
        task = self.render_tasks[axis]
        ...
        async def job():
            ...
            arr = await loop.run_in_executor(
                self.executor,
                self._compute_slice,
                axis, level0_index, level
            )
            self.views[axis].source = Image.fromarray(arr)

        self.render_tasks[axis] = asyncio.create_task(job())
